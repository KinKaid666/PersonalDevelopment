...
static public final class LifeGame {
  ...
  public LifeGame(int rows, int cols, double alivePercent) {
    initBoard(rows, cols, alivePercent) ;
  }

  public LifeGame(int rows, int cols) {
    initBoard(rows, cols, 0.5) ;
  }
  ...
  public void initBoard(int rows, int cols, double alivePct) {
    board_ = new ArrayList<List<Boolean>>(rows) ;
    for(int i = 0 ; i < rows ; i++) {
      List<Boolean> row = new ArrayList<Boolean>(cols) ;
      for(int j = 0 ; j < cols ; j++) {
        row.add(random_.nextDouble() <= alivePct) ;
      }
      board_.add(row) ;
    }
  }
