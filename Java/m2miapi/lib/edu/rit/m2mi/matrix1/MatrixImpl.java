...
// Unit:    Class edu.rit.m2mi.matrix1.MatrixImpl
//
...
// Exported operations inherited and implemented from interface Matrix.
	 ...
	 * invoking {@link Worker#putRow(int,float[]) <TT>putRow()</TT>} on a
	 * {@link Worker </TT>Worker<TT>} omnihandle.
	 */
	public synchronized void putRows()
		...
		for (int r = 0; r < rows; ++ r)
			{
			allWorkers.putRow (r, myRows[r]);
			}
		}

	/**
	 * Send all of this matrix's columns to the workers. Each column is sent by
	 * invoking {@link Worker#putColumn(int,float[]) <TT>putColumn()</TT>} on a
	 * {@link Worker </TT>Worker<TT>} omnihandle.
	 */
